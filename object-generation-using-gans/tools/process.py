...
def resize(src):
    ...
    if height != width:
        if a.pad:
            ...
            oh = (size - height) // 2
            ow = (size - width) // 2
            dst = im.pad(image=dst, offset_height=oh, offset_width=ow,
            ...
            oh = (height - size) // 2
            ow = (width - size) // 2
            dst = im.crop(image=dst, offset_height=oh, offset_width=ow,
                          target_height=size, target_width=size)
...
def blank(src):
    ...
    dst = src
    dst[offset:offset + size, offset:offset +
        size, :] = np.ones([size, size, 3])
    return dst
