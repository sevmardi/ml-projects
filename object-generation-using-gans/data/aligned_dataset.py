...
class AlignedDataset(BaseDataset):
    ...
    def initialize(self, opt):
        ...
        self.AB_paths, self.bbox_paths = make_dataset(
            self.dir_AB, self.dir_bbox)
    ...
    def __getitem__(self, index):
        ...
        bbox_x = max(
            int((bbox['x'] / self.opt.fineSize) * self.opt.loadSize), 0)
        bbox_y = max(
            int((bbox['y'] / self.opt.fineSize) * self.opt.loadSize), 0)
        bbox_w = max(
            int((bbox['w'] / self.opt.fineSize) * self.opt.loadSize), 0)
        bbox_h = max(
            int((bbox['h'] / self.opt.fineSize) * self.opt.loadSize), 0)

        if bbox_y <= h_offset or bbox_x <= w_offset:
            AB = Image.open(AB_path).convert('RGB')
            AB = AB.resize(
                (self.opt.fineSize * 2, self.opt.fineSize), Image.BICUBIC)
