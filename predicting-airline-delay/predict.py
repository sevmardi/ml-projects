...
if __name__ == '__main__':
    ...
    error = x[0][0] * 0.03
    print('Prediction from %d based on %s' %
          (years[-1], "{:,.0f}".format(predict_on_value[0])))
    print('Prediction for %d is %s +/- %s' % (predict_for_year,
                                              "{:,.0f}".format(x[0][0]), "{:,.0f}".format(error)))
    # Plot both values and predicted value
    plotpred(values, years, x, error,
             "Prediction for model %s, Epochs=%d, batch-size=%d" % (name, epochs, batches))
